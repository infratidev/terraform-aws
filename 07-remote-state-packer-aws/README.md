...
Estrutura utilizada para execução do script ```terraform-packer-aws.sh``` localizada no link abaixo:

[terraform-packer](https://github.com/infratidev/terraform-packer-aws)
...
s3-tfstate-packer.tf          => Criação e parametrização do S3 para deploy e network
variables.tf                  => variaveis
output.tf                     => Saída com o domain name e arn do bucket
```
